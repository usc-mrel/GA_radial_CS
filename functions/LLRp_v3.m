...
if overlap == 1
    Nshift = 9; %Always shift in 8 directions plus 1 without shift. 
else 
    Nshift = 1;
end 
...
if DO_SHIFT
    ...
    function res = resCalc(img)
        ...
        ind = patchInd(img,B,overlap);
        if ~overlap
            nP = numel(ind)/(Nt*B*B); %number of patches created
        else
            nP = numel(ind)/(Nt*B*B*Nshift);
        end 
        BLOCK = img(ind);
        %keyboard;
        if ~ overlap
            BLOCK = reshape(BLOCK, [B*B, nP, Nt]);
            BLOCK = permute(BLOCK, [1 3 2]);
        else 
            BLOCK = reshape(BLOCK, [B*B, nP, Nt, Nshift]);
            BLOCK = permute(BLOCK, [1 3 2 4]);
            BLOCK = reshape(BLOCK, [B*B, Nt, nP*Nshift]);
        end
        ...
        % Array Function Implementation %
            ...
            patchArray(nP*Nshift).data = []; %Initialize Structure
            for ip = 1:nP*Nshift
    ...
    %    %s = sign(s).*max(s-lambda,0);
        ...
        thres=sign(s).*(lambda).*(s.^(sign(s).*(p-1)));
